...
**最终方向（Day 2 定稿）**：语义搜索这条路测下来还是不够稳（AI 判断相关性本身就不确定）。改成回归结构化标签，但吸取之前"AI瞎猜地图英雄"的教训——**筛选环节完全不依赖 AI**：
- 数据库加了 4 个字段：`map`、`agent`、`kills`（三杀~七杀）、`special`（特殊标签数组，先只有"eco翻盘"一个，之后往 `src/lib/valorantData.ts` 的 `SPECIAL_OPTIONS` 数组里加就行）
- 上传时 AI 还是会看一眼截图猜地图/英雄/击杀数/特殊标签，但猜完之后**用下拉框和复选框展示**，猜错了用户直接改选项，比改文字快，也不会出现"文本对不上"的问题
- 首页把搜索框换成 4 个下拉筛选器（地图/英雄/击杀数/特殊标签），纯前端精确匹配过滤，不再调用 AI，秒出结果，也不会再有额度问题
- 删掉了 `/api/search` 这个路由（不再需要）

对应文件：`src/lib/valorantData.ts`（新建，存放地图/英雄/击杀数/特殊标签的选项列表）、`src/app/api/tag-clip/route.ts`、`src/app/upload/page.tsx`、`src/app/page.tsx`。见提交 `feat: 改成结构化标签+下拉筛选`。

**再收窄一次范围**：地图/英雄/击杀数/特殊标签这几个字段实测下来 AI 预测没有一次是准的，调整下拉框展示也没能挽救准确率问题本身。干脆把这几个字段的 AI 预测去掉，改成纯手动选择（默认不选中，用户自己点）。AI 只保留"看一眼截图生成标题建议"这一个用途——这是个开放式任务，容错率高，不会出现"明显认错专有名词"的尴尬。顺带把英雄名单里的"瑟符"改成玩家更常用的"零"。见提交 `feat: 移除AI对地图英雄击杀数的预测，只保留AI生成标题`。
...
**最终实现跟最初设计有几处调整**（详见 Day 2 的"变更记录"），简历描述按实际交付的版本写：

> 独立设计并开发个人游戏高光视频管理网站，基于 Next.js + Tailwind CSS 构建前端，使用 Supabase（PostgreSQL + 对象存储）实现数据持久化与文件托管；接入 Google Gemini 多模态 API，自动截取视频关键帧并生成内容标题建议，降低手动整理成本；支持按地图/英雄/击杀数等结构化标签精确筛选、私密分享链接生成，完整部署至 Vercel。

**步骤**：
1. 录一段 30 秒左右的操作屏幕（上传 → AI 生成标题建议 → 选标签保存 → 首页筛选 → 生成分享链接）
2. 截 2-3 张关键页面截图（首页列表、上传页、分享页）
3. 面试时可以重点讲的两个点：
   - **AI 落地的取舍过程**：最初想让 AI 精确识别地图/英雄这类专有名词，实测发现单帧图像分类不稳定（换了三种 prompt 策略、试过语义搜索），最后收敛成"AI 负责开放式的标题生成建议 + 结构化标签交给用户手动确认"——这个判断过程比"一次做对"更有面试话题
   - **权限踩坑**：Supabase 默认开启 RLS 导致读写被拦、Storage 桶权限跟表权限是两套独立规则、免费版存储桶 50MB 硬上限、Gemini 免费额度每天20次调用触发429——这些都是实际调试中定位到的问题
...
- AI 打标签方案几经调整：精确分类（地图/英雄/击杀数）→ 开放式描述+语义搜索 → 最终定为"结构化标签手动选择 + AI 只负责生成标题建议"，原因是单帧图像分类和 AI 相关性判断在实测中都不够稳定
- 首页筛选从"关键词搜索"改成"地图/英雄/击杀数/特殊标签四个下拉框精确匹配"，不再依赖 AI，速度和准确性都更好
- 额外加了原计划里没有的**删除视频**功能
- 界面从默认 Tailwind 浅色主题重新设计为深色 Industrial 风格（纯黑背景、等宽字体、信号红强调色）
- AI 模型从 Claude 换成 Gemini（成本考虑），中途又从 `gemini-flash-latest` 换成 `gemini-flash-lite-latest`（免费额度问题）

## 后续可选的延伸方向（不在这次范围内，供之后参考）

- 加多用户登录（Supabase Auth + RLS 按 user_id 隔离），让这个项目能给其他人用而不只是自己
- 把这个模块嵌入个人介绍网站的"爱好"板块里
- 支持批量上传/多游戏
- 如果想做微信小程序版本：需要换技术栈（uni-app/Taro），且涉及企业主体资质、内容审核、ICP备案，工作量远超当前 MVP，暂不建议投入
